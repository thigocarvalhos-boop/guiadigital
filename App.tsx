...
// Componentes simplificados para garantir renderização sem erros
const VisaoView = ({ user }: { user: UserProfile }) => (
  <div className="space-y-8">
    <div className="p-12 bg-gradient-to-r from-[#ff007a]/10 to-transparent border-2 border-white/5 relative">
      <h1 className="text-7xl font-black text-white mb-2 leading-none" style={{fontFamily: 'Archivo Black'}}>{user.name}</h1>
      <p className="mono text-[#00f2ff] font-bold text-sm tracking-widest uppercase">Operador de Ativos // Node: Porto Digital</p>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {Object.entries(user.matrix).map(([key, val]) => (
        <div key={key} className="p-8 bg-slate-900/40 border border-white/5">
          ...
          <div className="h-2 w-full bg-black/80">
            <div className="h-full bg-gradient-to-r from-[#ff007a] to-[#00f2ff]" style={{ width: `${val * 10}%` }}></div>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      {TRACKS.map(track => (
        <div 
          ...
          className="p-10 cursor-pointer bg-slate-900/20 border-2 border-white/5 hover:border-[#00f2ff] transition-all flex flex-col justify-between h-[360px]"
        >
          <div>
            ...
            <p className="text-slate-400 text-sm leading-relaxed italic">"{track.lessons[0].theory}"</p>
          </div>
          <button className="bg-[#ff4d00] text-white py-4 px-10 text-[11px] font-bold tracking-widest">INICIAR OPERAÇÃO</button>
        </div>
      ))}
    </div>
  </div>
...
const MissionEnvironment = ({ lesson, onClose, onAudit }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4">
      <div className="w-full max-w-5xl bg-[#020617] border-2 border-[#ff007a]/20 flex flex-col min-h-[80vh]">
        <div className="p-8 border-b-4 border-black flex justify-between items-center">
          <span className="font-bold text-[#ff007a] text-sm tracking-widest">OPER::LAB_V2</span>
          <button onClick={onClose} className="text-slate-500 hover:text-white"><i className="fa-solid fa-xmark text-3xl"></i></button>
        </div>

        <div className="flex-1 p-8 md:p-16">
          {step === 1 && (
            <div className="space-y-10">
              <h1 className="text-7xl font-black text-white italic uppercase" style={{fontFamily: 'Archivo Black'}}>MISSÃO DE<br/><span className="text-[#00f2ff]">VALIDAÇÃO</span></h1>
              <div className="p-10 bg-black/60 border border-[#00f2ff]/10">
                 <p className="text-3xl font-black text-white">{lesson.labPrompt}</p>
              </div>
              <button onClick={() => setStep(2)} className="w-full bg-[#ff4d00] py-8 text-xl font-bold">INICIAR ENTREGA TÉCNICA</button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-8 flex flex-col h-full">
              <textarea 
                ...
                className="flex-1 w-full min-h-[300px] bg-black border-4 border-slate-900 p-8 text-xl text-[#00f2ff] font-mono outline-none focus:border-[#ff007a]"
              />
              <button disabled={!input || loading} onClick={handleAudit} className="w-full bg-[#ff4d00] py-8 text-xl font-bold">
          ...
          {step === 3 && result && (
            <div className="text-center space-y-10">
              <div className="text-[120px] font-black italic text-white" style={{fontFamily: 'Archivo Black'}}>
                {result.score}<span className="text-[#ff007a] text-5xl">/10</span>
              </div>
              <div className="p-10 bg-white/5 border border-[#ff007a]/30 text-left">
                <p className="text-white text-2xl font-bold italic font-mono">"{result.feedback}"</p>
              </div>
              <button onClick={onClose} className="w-full bg-[#ff4d00] py-8 text-2xl font-bold">ARQUIVAR NO DOSSIÊ</button>
            </div>
          )}
        </div>
      </div>
  ...
  <div className="space-y-12">
    ...
    <div className="grid grid-cols-1 gap-6">
       {dossier.map((item: any, i: number) => (
         <div key={i} className="p-10 bg-slate-900/20 border border-white/5 flex justify-between items-center">
            <div>
               <h4 className="text-3xl font-black text-white" style={{fontFamily: 'Archivo Black'}}>{item.lessonTitle}</h4>
               <p className="text-sm text-slate-500 font-mono mt-2">DATA: {item.date} | SCORE: {item.audit.score}</p>
            </div>
            <button className="bg-[#ff4d00] px-8 py-3 font-bold text-xs">DETALHES</button>
         </div>
       ))}
       {dossier.length === 0 && <p className="text-slate-700 font-bold uppercase tracking-widest text-center py-20">Dossiê Vazio.</p>}
    </div>
  </div>
);
...
const ManifestoView = () => (
  <div className="max-w-3xl mx-auto space-y-12 text-slate-200">
    ...
    <div className="p-12 bg-black/40 border border-white/10 space-y-8">
       <p className="text-xl font-bold italic leading-relaxed">01. Não é um curso. É um sistema de hackeamento de carreira.</p>
       <p className="text-xl font-bold italic leading-relaxed">02. Rigor é respeito. O mercado real não perdoa amadorismo.</p>
       <p className="text-xl font-bold italic leading-relaxed">03. Periferia é o centro da nova economia criativa.</p>
    </div>
  </div>
);
...
const InitStreetSession = ({ onComplete }: any) => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center p-8 text-center">
      <div className="w-24 h-24 bg-white rotate-6 mb-8 flex items-center justify-center shadow-[10px_10px_0_#ff007a]">
      ...
      <div className="w-full max-w-md space-y-8">
        <input 
          ...
          className="w-full bg-transparent border-b-8 border-slate-900 py-6 text-center text-4xl font-black text-white outline-none focus:border-[#ff007a] uppercase italic"
        />
        <button 
          disabled={!name || loading}
          onClick={handleStart}
          className="w-full bg-[#ff4d00] py-8 text-xl font-bold shadow-[0_10px_30px_rgba(255,77,0,0.3)]"
        >
